                   ...
                   double *resp, int *ns)
{
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        log_trace(3,"sat=%4d azel=%5.1f %4.1f res=%7.3f sig=%5.3f\n",obs[i].sat,
    ...
    /* constraint receiver position */
    if (opt->mode==PMODE_FIXED) {
        if (norm(opt->ru,3)) {
            for (i=0;i<3;i++) {
                v[nv]=opt->ru[i]-x[i];
                for (j=0;j<NX;j++) H[j+nv*NX]=j==i?1.0:0.0;
                var[nv++]=SQR(0.01);
            }
        }
    }
                  ...
                  const prcopt_t *opt, const double *v, int nv, int nx, char *msg)
{
    ...
    if (nv>nx&&vv>chisqr[nv-nx-1]) {
        log_trace(1,"chi-square error nv=%d vv=%.1f cs=%.1f\n",nv,vv,chisqr[nv-nx-1]);
        return 0;
    }
    /* large GDOP check */
    for (i=ns=0;i<n;i++) {
    ...
    if (dop[0]<=0.0||dop[0]>opt->maxgdop) {
        log_trace(1,"gdop error nv=%d gdop=%.1f\n",nv,dop[0]);
        return 0;
    }
    return 1;
}
                  ...
                  char *msg)
{
    ...
    rs=zeros(n,6); dts=mat(n,2); var=mat(n,1);
    azel_=mat(n,2); resp=zeros(n,1);
