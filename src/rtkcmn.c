...
const prcopt_t prcopt_default_ppp={ /* defaults processing options */
        PMODE_PPP_STATIC,0,3,SYS_GPS|SYS_CMP|SYS_QZS|SYS_GAL|SYS_GLO,   /* mode,soltype,nf,navsys */
        5.0*D2R,{{0,0}},            /* elmin,snrmask */
        1,1,1,1,                    /* sateph,modear,glomodear,bdsmodear */
        5,0,3,1,                    /* maxout,minlock,minfix,armaxiter */
        3,3,0,1,                    /* estion,esttrop,dynamics,tidecorr */
        3,0,0,0,0,                  /* niter,codesmooth,intpref,sbascorr,sbassatsel */
        0,0,                        /* rovpos,refpos */
        {100.0,100.0,100.0},        /* eratio[] */
        {100.0,0.003,0.003,0.0,1.0}, /* err[] */
        {60.0,0.03,0.3},            /* std[] */
        {1E-4,1E-4,1E-4,1E-1,1E-2,1E-8}, /* prn[] */
        5E-12,                      /* sclkstab */
...
extern double sat2freq_e(int sat, uint8_t code)
{
    int i,fcn=0,sys,prn;
    ...
    if (sys==SYS_GLO) fcn=glo_fcn[prn-1];
    return code2freq(sys,code,fcn);
}
/* set code priority -----------------------------------------------------------
...
extern double *mat(int n, int m)
{
    ...
    if (!(p=(double *)malloc(n*m*sizeof(double)))) {
...
extern void outer(const double *a, const double *b, int na, int nb, double *A)
{
    int i,j;
    for (i=0;i<na;i++) {
        for (j=0;j<nb;j++) A[i+j*na]=a[i]*b[j];
    }
}
/* copy matrix -----------------------------------------------------------------
* copy matrix
...
extern void time2str_(gtime_t t, char *s, int n)
{
    double ep[6];
    ...
    if (1.0-t.sec<0.5/pow(10.0,n)) {t.time++; t.sec=0.0;};
    time2epoch(t,ep);
    sprintf(s,"%04.0f_%02.0f_%02.0f_%02.0f_%02.0f",ep[0],ep[1],ep[2],ep[3],ep[4]);
}
/* get time string -------------------------------------------------------------
* get time string
* args   : gtime_t t        I   gtime_t struct
...
extern char *time_str_(gtime_t t, int n)
{
    static char buff[64];
    time2str_(t,buff,n);
    return buff;
...
*          double *erpv     I   erp values {xp,yp,ut1_utc,lod,tai_utc} (rad,rad,s,s/d,s)
...
*          not thread-safe
*-----------------------------------------------------------------------------*/
extern void eci2ecef(gtime_t tutc, const double *erpv, double *U, double *dU, double *gmst)
{
    const double ep2000[]={2000,1,1,12,0,0};
    ...
    double R1[9],R2[9],R3[9],R[9],W[9],N[9],P[9],NP[9],S[9],O[9],omg;
    int i;
    ...
    eps=(84381.447996-46.8150*t-0.00059*t2+0.001813*t3)*AS2R;
    ...
    S[1]=-1.0; S[3]=1.0;
    omg=OMGE-0.843994809*1E-9*erpv[3];
    matmul("NN",3,3,3,omg,S,R3,0.0,O);

    matmul("NN",3,3,3,1.0,W,O ,0.0,R); /* W=Ry(-xp)*Rx(-yp) */
    matmul("NN",3,3,3,1.0,R,NP,0.0,dU_); /* dU=W*O(gast)*N*P */
...
extern int geterp(const erp_t *erp, gtime_t time, double *erpv)
{
    ...
    double mjd,day,a;
    static gtime_t etime={0};
    static double eerpv[6]={0};
    int i,j,k;
    ...
    if (mjd>=erp->data[erp->n-1].mjd) {
        ...
        erpv[4]=erp->data[erp->n-1].tai_utc;
        return 1;
    }
    ...
    erpv[4]=erp->data[j].tai_utc;

    matcpy(eerpv,erpv,1,5);
    etime=time;
    return 1;
}
/* compare ephemeris ---------------------------------------------------------*/
...
{
    if (opt&0x01) {if (nav->eph ) free(nav->eph ); nav->eph =NULL; nav->n =nav->nmax =0;}
    if (opt&0x02) {if (nav->geph) free(nav->geph); nav->geph=NULL; nav->ng=nav->ngmax=0;}
    if (opt&0x04) {if (nav->seph) free(nav->seph); nav->seph=NULL; nav->ns=nav->nsmax=0;}
    if (opt&0x08) {if (nav->peph) free(nav->peph); nav->peph=NULL; nav->ne=nav->nemax=0;}
    if (opt&0x10) {if (nav->pclk) free(nav->pclk); nav->pclk=NULL; nav->nc=nav->ncmax=0;}
    if (opt&0x20) {if (nav->alm ) free(nav->alm ); nav->alm =NULL; nav->na=nav->namax=0;}
    if (opt&0x40) {if (nav->tec ) free(nav->tec ); nav->tec =NULL; nav->nt=nav->ntmax=0;}
}
/* expand file path ------------------------------------------------------------
...
extern int expath(const char *path, char *paths[], int nmax)
{
    ...
    if (strcmp(file.cFileName,".")&&strcmp(file.cFileName,"..")) sprintf(paths[n++],"%s%s",dir,file.cFileName);
    while (FindNextFile(h,&file)&&n<nmax) {
        if (file.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY) continue;
        if (!strcmp(file.cFileName,".")) continue;
        if (!strcmp(file.cFileName,"..")) continue;
                       ...
                       double *rmoon, double *gmst)
{
    ...
    /* eci to ecef transformation matrix */
    eci2ecef(tutc,erpv,U,NULL,&gmst_);
...
/* RMS-------------------------------------------------------------------------*/
extern double rms(const double *val, int n)
{
    double s=0.0;
    int i;
